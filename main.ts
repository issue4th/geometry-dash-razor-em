namespace SpriteKind {
    ...
    export const backwards_player = SpriteKind.create()
}
scene.onOverlapTile(SpriteKind.selector, assets.tile`myTile26`, function (sprite, location) {
    tiles.loadMap(tiles.createMap(tilemap`level25`))
    setup_Level()
})
scene.onOverlapTile(SpriteKind.selector, assets.tile`myTile49`, function (sprite, location) {
...
controller.up.onEvent(ControllerButtonEvent.Pressed, function () {
    if (ballrazor) {
        if (ballrazor.tileKindAt(TileDirection.Bottom, assets.tile`myTile8`) || (ballrazor.tileKindAt(TileDirection.Bottom, assets.tile`myTile36`) || ballrazor.tileKindAt(TileDirection.Bottom, assets.tile`myTile10`))) {
            if (ballrazor) {
                ballrazor.vy = -200
